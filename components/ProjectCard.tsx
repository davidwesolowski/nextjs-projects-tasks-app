...
const PROJECT_WITH_TASKS = Prisma.validator<Prisma.ProjectArgs>()({
    include: {
        tasks: true
    }
})

type ProjectWithTasks = Prisma.ProjectGetPayload<typeof PROJECT_WITH_TASKS>;  
...
export const ProjectCard: FC<{project: ProjectWithTasks}> = ({ project}) => {
    const completedCount = project.tasks.map((task) => task.status === TASK_STATUS.COMPLETED).length;
